...
{
  QString path = ProfileManager::activeProfile().dataDir("abyssfin.conf");
  if (path.isEmpty())
    return QVariant();
...
void SettingsComponent::load()
{
  ...
  loadConf(ProfileManager::activeProfile().dataDir("abyssfin.conf"), false);
...
bool SettingsComponent::resetAndSaveOldConfiguration()
{
  ...
  return settingsFile.rename(ProfileManager::activeProfile().dataDir("abyssfin.conf.old"));
}
